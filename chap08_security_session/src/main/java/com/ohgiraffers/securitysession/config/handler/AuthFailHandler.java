...
* 우리는 AuthenticationFailureHandler를 구현해야 하지만 기존에 구현이 되있는 SimpleUrlAuthenticationFailureHandler를 상속받아
...
public class AuthFailHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        if (exception instanceof BadCredentialsException){
            ...
            // InternalAuthenticationServiceException 오류는 서버에서 사용자 정보를 검증하는 과정에서 발생하는 에러이다.
            errorMessage = "서버에서 오류가 발생되었습니다.";
        } else if (exception instanceof UsernameNotFoundException) {
            // db에 사용자의 정보가 없는 경우 발생하는 오류이다.
            errorMessage = "존재하지 않는 이메일 입니다.";
        } else if (exception instanceof AuthenticationCredentialsNotFoundException) {
            // 보안 컨텍스트에 인증 객체가 존재하지 않거나 인증 정보가 없는 상태에서 보안처리된 리소스에 접근하는 경우 발생하는 오류
        ...
        setDefaultFailureUrl("/auth/fail?message="+errorMessage); // 요청이 실패 했을때 보낼 경로
