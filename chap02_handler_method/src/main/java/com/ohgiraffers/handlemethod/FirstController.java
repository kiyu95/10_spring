...
public class FirstController {
    ...
    * 위의 first는 template 하위의 디렉토리를 의미한다.
    * */
    ...
    * 상위 타입은 ServletRequest, ServletResponse도 사용 가능하다.
    ...
    * */
    ...
    public String registMenu(Model model, WebRequest request){
        // WebRequest 객체의 getParameter 등의 메서드를 통해 클라이언트로 부터 전달 된 파라미터를 가져올 수 있다.
    ...
    * @ModelAttribute("모델에담을key")을 지정할 수 있으며, 지정하지 않으면 타입의 앞글자를 소문자로 한 네이밍 규칙을 따른다.
    * meuuDTO로 담긴다.
    * */
    @PostMapping("search")
    ...
    * */
    ...
    // 클래스레벨에 @SessionAttributes 어노테이션을 이용하여 세션에 값을 담을 key를 설정해 두면
    ...
    * json으로 전달하는 경우 jackson 컨버터로 자동 파싱하여 사용할 수 있다.
    *
    * 주로 restApi 작성 시 많이 사용되며, 일반적인 form 전송을 할 때는 거의 사용되지 않는다.
    *
    * 추가적으로 헤더에 대한 정보도
    ...
    * */
    ...
    public void bodyTest(@RequestBody String body,
                         @RequestHeader("content-type") String contentType, // 꺼내고자 하는 header의 key를 넣는다.
